---
description: "Create an architecture decision document using BMAD methodology (8-step interactive workflow)"
---

# Create Architecture

You are starting the **Create Architecture** workflow — an 8-step interactive process to create a comprehensive Architecture Decision Document using BMAD methodology.
...
1. **Language Preferences** *(always ask before anything else)*:
   ...
   2. **Document output language** — What language should the architecture document be written in? (e.g. English, Vietnamese, French...)
...
3. **Load Skill**: Read and follow the `architecture/create` skill instructions.

4. **Establish Step File Path**: Step files for this workflow are located at:
   `${CLAUDE_PLUGIN_ROOT}/skills/architecture/create/references/`

5. **Begin Workflow**: Read fully and follow `${CLAUDE_PLUGIN_ROOT}/skills/architecture/create/references/step-01-init.md`
...
- A PRD is required as input — the workflow will prompt if none is found
