---
description: "Generate styled draw.io (.drawio) files from architecture shard document content with C4 conventions. Can run sequentially, in parallel, or interactively."
agent: agent
---
...
6. **Route Based on Choice**:
   ...
   - **If B (Parallel)**: Spawn **all** `c4-diagram-generator` subagents simultaneously — one per shard document file. Use **parallel agent invocations in a single message** so they run concurrently. Each subagent receives a single file path. After all complete, combine their summary tables into one final table and present to the user.
   ...
   - **If C (Interactive)**: Read `.github/skills/architecture-diagrams/SKILL.md` and follow its instructions. For each shard document:
