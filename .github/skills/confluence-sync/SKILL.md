...
```bash
plugins/scrum-master/scripts/push-md-to-confluence.sh <file.md> "description of changes"
```
...
```bash
plugins/scrum-master/scripts/md-to-confluence-storage.sh <file.md> /tmp/output.html
```
...
```bash
plugins/scrum-master/scripts/pull-confluence-to-md.sh <page_id> [output.md]
```
...
for f in <directory>/*.md; do
  echo "--- Pushing: $f ---"
  plugins/scrum-master/scripts/push-md-to-confluence.sh "$f" "Batch sync"
done
```
